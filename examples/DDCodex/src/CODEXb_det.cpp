...
static Ref_t create_element(Detector& description, xml_h e, Ref_t sens)  {
  ...
  for(xml_coll_t i(x_det, _U(station)); i; ++i, ++num_super) {
    ...
    //double     face_thick  = st_dist/2.0*(2-1) + st_thick*2;
    string     super_name   = station.nameStr();
    Box        super_box(super_thick,super_width+50.0,super_height+50.0);
    ...
    //double     face_x = -(super_height*(2-1) + st_thick*2) + st_thick;
    ...
    for (int j=0; j < super_repeat; ++j )    {
      ...
      if( super_name.compare("inner_station") == 0 ) pv = super_vol.placeVolume(st_vol, Position(inner_x, 0, 0));
